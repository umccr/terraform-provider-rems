/*
REMS API

REMS API Services

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package remsclient

import (
	"bytes"
	"encoding/json"
	"fmt"
)

// checks if the InviteReviewerCommand type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &InviteReviewerCommand{}

// InviteReviewerCommand struct for InviteReviewerCommand
type InviteReviewerCommand struct {
	ApplicationId int64                         `json:"application-id"`
	Comment       *string                       `json:"comment,omitempty"`
	Attachments   []CommandAttachment           `json:"attachments,omitempty"`
	Reviewer      InviteReviewerCommandReviewer `json:"reviewer"`
}

type _InviteReviewerCommand InviteReviewerCommand

// NewInviteReviewerCommand instantiates a new InviteReviewerCommand object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInviteReviewerCommand(applicationId int64, reviewer InviteReviewerCommandReviewer) *InviteReviewerCommand {
	this := InviteReviewerCommand{}
	this.ApplicationId = applicationId
	this.Reviewer = reviewer
	return &this
}

// NewInviteReviewerCommandWithDefaults instantiates a new InviteReviewerCommand object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInviteReviewerCommandWithDefaults() *InviteReviewerCommand {
	this := InviteReviewerCommand{}
	return &this
}

// GetApplicationId returns the ApplicationId field value
func (o *InviteReviewerCommand) GetApplicationId() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.ApplicationId
}

// GetApplicationIdOk returns a tuple with the ApplicationId field value
// and a boolean to check if the value has been set.
func (o *InviteReviewerCommand) GetApplicationIdOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ApplicationId, true
}

// SetApplicationId sets field value
func (o *InviteReviewerCommand) SetApplicationId(v int64) {
	o.ApplicationId = v
}

// GetComment returns the Comment field value if set, zero value otherwise.
func (o *InviteReviewerCommand) GetComment() string {
	if o == nil || IsNil(o.Comment) {
		var ret string
		return ret
	}
	return *o.Comment
}

// GetCommentOk returns a tuple with the Comment field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InviteReviewerCommand) GetCommentOk() (*string, bool) {
	if o == nil || IsNil(o.Comment) {
		return nil, false
	}
	return o.Comment, true
}

// HasComment returns a boolean if a field has been set.
func (o *InviteReviewerCommand) HasComment() bool {
	if o != nil && !IsNil(o.Comment) {
		return true
	}

	return false
}

// SetComment gets a reference to the given string and assigns it to the Comment field.
func (o *InviteReviewerCommand) SetComment(v string) {
	o.Comment = &v
}

// GetAttachments returns the Attachments field value if set, zero value otherwise.
func (o *InviteReviewerCommand) GetAttachments() []CommandAttachment {
	if o == nil || IsNil(o.Attachments) {
		var ret []CommandAttachment
		return ret
	}
	return o.Attachments
}

// GetAttachmentsOk returns a tuple with the Attachments field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InviteReviewerCommand) GetAttachmentsOk() ([]CommandAttachment, bool) {
	if o == nil || IsNil(o.Attachments) {
		return nil, false
	}
	return o.Attachments, true
}

// HasAttachments returns a boolean if a field has been set.
func (o *InviteReviewerCommand) HasAttachments() bool {
	if o != nil && !IsNil(o.Attachments) {
		return true
	}

	return false
}

// SetAttachments gets a reference to the given []CommandAttachment and assigns it to the Attachments field.
func (o *InviteReviewerCommand) SetAttachments(v []CommandAttachment) {
	o.Attachments = v
}

// GetReviewer returns the Reviewer field value
func (o *InviteReviewerCommand) GetReviewer() InviteReviewerCommandReviewer {
	if o == nil {
		var ret InviteReviewerCommandReviewer
		return ret
	}

	return o.Reviewer
}

// GetReviewerOk returns a tuple with the Reviewer field value
// and a boolean to check if the value has been set.
func (o *InviteReviewerCommand) GetReviewerOk() (*InviteReviewerCommandReviewer, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Reviewer, true
}

// SetReviewer sets field value
func (o *InviteReviewerCommand) SetReviewer(v InviteReviewerCommandReviewer) {
	o.Reviewer = v
}

func (o InviteReviewerCommand) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o InviteReviewerCommand) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["application-id"] = o.ApplicationId
	if !IsNil(o.Comment) {
		toSerialize["comment"] = o.Comment
	}
	if !IsNil(o.Attachments) {
		toSerialize["attachments"] = o.Attachments
	}
	toSerialize["reviewer"] = o.Reviewer
	return toSerialize, nil
}

func (o *InviteReviewerCommand) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"application-id",
		"reviewer",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varInviteReviewerCommand := _InviteReviewerCommand{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varInviteReviewerCommand)

	if err != nil {
		return err
	}

	*o = InviteReviewerCommand(varInviteReviewerCommand)

	return err
}

type NullableInviteReviewerCommand struct {
	value *InviteReviewerCommand
	isSet bool
}

func (v NullableInviteReviewerCommand) Get() *InviteReviewerCommand {
	return v.value
}

func (v *NullableInviteReviewerCommand) Set(val *InviteReviewerCommand) {
	v.value = val
	v.isSet = true
}

func (v NullableInviteReviewerCommand) IsSet() bool {
	return v.isSet
}

func (v *NullableInviteReviewerCommand) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInviteReviewerCommand(val *InviteReviewerCommand) *NullableInviteReviewerCommand {
	return &NullableInviteReviewerCommand{value: val, isSet: true}
}

func (v NullableInviteReviewerCommand) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInviteReviewerCommand) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
